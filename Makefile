...
CLICKHOUSE_URL='clickhouse://${CLICKHOUSE_HOST}:${CLICKHOUSE_PORT}?username=${CLICKHOUSE_USER}&password=${CLICKHOUSE_PASSWORD}&database=${CLICKHOUSE_DB}}'
build:
...
migrate-clickhouse-up:
	migrate -database ${CLICKHOUSE_URL} -path migrations/clickhouse up

migrate-clickhouse-down:
	migrate -database ${CLICKHOUSE_URL} -path migrations/clickhouse down

create-new-migration-clickhouse: # make create-new-migration-clickhouse name=file_name
	migrate create -ext sql -dir migrations/clickhouse -seq $(name)
